    ...
    if CONF_VISUAL in config:
        ...
        if CONF_MIN_TEMPERATURE in visual_config:
            ...
            if min_temp < TCLAC_MIN_TEMPERATURE:
                raise cv.Invalid(f"Mínima de {min_temp} abaixo do permitido {TCLAC_MIN_TEMPERATURE}")
        else:
        ...
        if CONF_MAX_TEMPERATURE in visual_config:
            ...
            if max_temp > TCLAC_MAX_TEMPERATURE:
                raise cv.Invalid(f"Máxima de {max_temp} acima do permitido {TCLAC_MAX_TEMPERATURE}")
        else:
        ...
        if CONF_TEMPERATURE_STEP in visual_config:
            ...
            if ((int)(temp_step * 2)) / 2 != temp_step:
                raise cv.Invalid(f"Passo de temperatura {temp_step} deve ser múltiplo de 1")
        else:
...
@automation.register_action("climate.tclac.display_on", DisplayOnAction, cv.Schema, synchronous=False)
@automation.register_action("climate.tclac.display_off", DisplayOffAction, cv.Schema, synchronous=False)
async def display_action_to_code(config, action_id, template_arg, args):
    paren = await cg.get_variable(config[CONF_ID])
    var = cg.new_Pvariable(action_id, template_arg, paren)
    return var

@automation.register_action("climate.tclac.beeper_on", BeeperOnAction, cv.Schema, synchronous=False)
@automation.register_action("climate.tclac.beeper_off", BeeperOffAction, cv.Schema, synchronous=False)
async def beeper_action_to_code(config, action_id, template_arg, args):
    paren = await cg.get_variable(config[CONF_ID])
    var = cg.new_Pvariable(action_id, template_arg, paren)
    return var

@automation.register_action("climate.tclac.module_display_on", ModuleDisplayOnAction, cv.Schema, synchronous=False)
@automation.register_action("climate.tclac.module_display_off", ModuleDisplayOffAction, cv.Schema, synchronous=False)
async def module_display_action_to_code(config, action_id, template_arg, args):
    paren = await cg.get_variable(config[CONF_ID])
    var = cg.new_Pvariable(action_id, template_arg, paren)
    return var

@automation.register_action("climate.tclac.force_mode_on", ForceOnAction, cv.Schema, synchronous=False)
@automation.register_action("climate.tclac.force_mode_off", ForceOffAction, cv.Schema, synchronous=False)
